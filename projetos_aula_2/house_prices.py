...
Ideal se você tem alguma experiência com R ou Python e noções básicas de aprendizado de máquina. Esta é uma competição perfeita para estudantes de ciência 
de dados que concluíram um curso on-line em aprendizado de máquina e desejam expandir seu conjunto de habilidades antes de tentar uma competição oficial. 
Peça a um comprador para descrever a casa dos seus sonhos, e eles provavelmente não começarão com a altura do teto do porão ou a proximidade de uma ferrovia 
leste-oeste. Mas o conjunto de dados desta competição prova que o número de quartos influencia mais as negociações de preços do que uma cerca branca apenas. 
Com 79 variáveis explicativas que descrevem diversos aspectos de imóveis residenciais em Ames, Iowa. Essa competição desafia você a prever o preço final de cada casa.

Habilidades requeridas:

Feature Engineering;
Técnicas avançadas de regressão, como random forest e gradient boosting;


SalePrice - o preço de venda da propriedade em dólares. Essa é a variável target.
...
BsmtExposure: paredes da cave ou do jardim
...
FullBath: banheiros completos acima da classe
HalfBath: Ha Se banheiros acima da classe
Bedroom: Número de quartos acima do porão
...
3SsnPorch: Área de alpendre de três estações em pés quadrados
ScreenPorch: Área de alpendre de tela em pés quadrados
PoolArea: Área de piscina em pés quadrados
PoolQC: Qualidade da piscina
Fence: qualidade da cerca
MiscFeature: Recurso diversificado não coberto em outras categorias
MiscVal: $ Valor de recurso variado
MoSold: vendido ao mes
YrSold: vendido ao ano
...
# ================================= Aquisição dos dados com API Kaggle ==========================================================
...
'''
A inclinação indica o gradiente de uma linha e o intercepto indica o local onde ele cruza um eixo. A inclinação e o intercepto definem a relação linear entre duas variáveis, e pode ser usado para estimar uma taxa média de mudança. Quanto maior a magnitude da inclinação, mais íngreme é a linha e maior a taxa de mudança. Ao examinar a equação de uma linha, você pode determinar rapidamente sua inclinação e intercepto y (onde a linha cruza o eixo y).

A inclinação é 5 positiva. Quando x aumenta em 1, y aumenta em 5. A interceptação y é 2.

Os coeficientes de regressão representam a mudança média na variável resposta para uma unidade de mudança na variável preditora.
'''
...
# ================================= Preparação para envio aos padrões kaggle ==========================================================

error_rate = np.array([np.sqrt(metrics.mean_squared_error(y_test, predictions)),np.sqrt(metrics.mean_squared_error(y_test, clf_pred)),np.sqrt(metrics.mean_squared_error(y_test, dtr_pred)),np.sqrt(metrics.mean_squared_error(y_test, svr_pred)),np.sqrt(metrics.mean_squared_error(y_test, rfr_pred))])
...
!kaggle competitions submit -c house-prices-advanced-regression-techniques -f submit.csv -m "competição 3 by qoda!"
